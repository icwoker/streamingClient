  ...
  <div class="flex justify-center flex-col gap-8">
    <div class="flex flex-wrap">
      ...
      <div class="w-full">
        ...
        <div v-else-if="fansList.length === 0" class="text-center py-12 text-gray-500">
          暂无粉丝
        </div>
        
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <!-- 粉丝卡片 -->
          <div v-for="fan in fansList" :key="fan.id" class="border-none rounded-lg p-4 flex flex-col shadow-sm hover:shadow-md transition-shadow">
            <!-- 粉丝基本信息 -->
            <div class="flex items-center mb-3">
              <img :src="`${apiBaseUrl}/${fan.avatar_url}`" alt="头像" class="w-16 h-16 rounded-full object-cover mr-4">
              <div class="flex flex-col flex-1">
                <div class="font-bold text-lg">{{ fan.name }}</div>
                <div class="text-xs text-gray-500">关注时间: {{ formatDate(fan.follow_time) }}</div>
              </div>
            </div>
            
            <!-- 粉丝简介 -->
            <div class="text-sm text-gray-700 mb-4 line-clamp-2">{{ fan.bio || '这个人很懒，什么也没留下' }}</div>
            ...
            <div class="flex justify-between items-center">
              <div>
                ...
                <span v-else class="px-2 py-2 bg-gray-100 text-gray-700 rounded-sm text-xs">
                  已互相关注
                </span>
              </div>
...
import { addUserToLiveBanner, removeUserFromLiveBanner } from '@/API/liveBanner';
...
const blockFan = async (fan: fanType) => {
  ...
  try{
    const response = await addUserToLiveBanner(fan.id);
    if(response.status === 200){
